...
The Specials section (`#specials`) in `Sections.jsx` reads `FT_DATA.specials[0]` (savory) and `FT_DATA.specials[1]` (sweet) from `data.js`. Source of truth is the restaurant's weekly Instagram post.

**Source post (current):** https://www.instagram.com/p/DZPmJyujUi-/
The shortcode (`DZPmJyujUi-`) changes weekly. Always pull the latest from `data.js → FT_DATA.sourcePost`, or ask the user for the new shortcode.

**Carousel children (CRITICAL — read this before pulling images):**

Instagram carousels have 2+ images. The `/p/<shortcode>/` URL renders slide 1 as `og:image`, then loads carousel children lazily on user interaction. **Do NOT** try to extract carousel slide 2 from:

- `<img>` tags on the public `/p/<shortcode>/` page (logged out) — only og:image (slide 1) is in DOM
- `<img>` tags on the public `/p/<shortcode>/embed/` or `/embed/captioned` pages — these load ONLY slide 1 of the current post + thumbnails of OTHER (sibling) posts from the same account. The non-first `t39.30808-6` images you'll see in DOM look like carousel children but they are PREVIOUS POSTS. The `?img_index=2` query param does NOT change embed image rendering.
- Querying via the logged-in `instagram.com/p/<shortcode>/` page DOM without scrolling/hovering — carousel slides 2+ aren't in DOM until user interacts

**This bug shipped once.** Sweet card displayed last week's savory image (`712314417_...` dated May 30) instead of this week's sweet (`715501857_...` dated June 6). Don't repeat.

**Correct path — read the embed page's data store:**

```javascript
// 1. Navigate any tab to the embed (captioned variant exposes the data):
//    https://www.instagram.com/p/<shortcode>/embed/captioned
// 2. Wait for hydration (~3-4 sec). Then read:
const media = window.__additionalData.extra.data.shortcode_media;
const children = media.edge_sidecar_to_children.edges;  // array of carousel items
const savory = children[0].node;
const sweet  = children[1].node;
// 3. Each node has display_resources[] — pick the highest-res entry:
const savoryUrl = savory.display_resources[savory.display_resources.length - 1].src;
const sweetUrl  = sweet.display_resources[sweet.display_resources.length - 1].src;
// 4. Caption (savory + sweet name + desc) is in media.edge_media_to_caption.edges[0].node.text
//    or scrape the captioned-embed body text (.body.innerText) — same content.
```

**Assertions to run BEFORE committing pulled images:**

- `children.length >= 2` — post must be a carousel of 2+ items.
- For each child: `node.is_video === false`. If video, STOP and ask user.
- Compare child `node.id`s to the shortcode_media `id` to confirm same post (sibling-post leak guard). Children's IDs share a numeric prefix with the parent's `id`.
- Confirm each downloaded image filename uses the **fetched** image's bytes (use the API path, not a guess from DOM).
- After saving, log `sha256` of each file. Compare against the previous week's saves — if savory or sweet hash matches the previous week, it's the wrong image, STOP.

**Download mechanism (multi-download workaround):**

Chrome blocks consecutive programmatic downloads from a single tab. Open a fresh tab per image:

```javascript
// For each carousel child, open a NEW tab via tabs_create_mcp,
// navigate to the embed URL, run the fetch+anchor.click() flow,
// wait for the file to land in ~/Downloads, then close the tab.
```

**Image-size flow:**

The CDN may serve smaller images than `display_resources` declares (810x1014 instead of 1080x1355 in some cases). Acceptable. Then:

```bash
WK=$(date +%Y-%m-%d)  # weekOf in repo: same date as sync
ASSETS=~/flytrap-website/assets/specials
cp ~/Downloads/specials-savory-correct.jpg $ASSETS/week-$WK-savory.jpg
cp ~/Downloads/specials-sweet-correct.jpg  $ASSETS/week-$WK-sweet.jpg
cd $ASSETS
# crop center square then upscale to 1080x1080 (AUTOMATION.md target)
W=$(sips -g pixelWidth week-$WK-savory.jpg | awk '/pixelWidth/{print $2}')
sips --cropToHeightWidth $W $W week-$WK-savory.jpg > /dev/null
W=$(sips -g pixelWidth week-$WK-sweet.jpg | awk '/pixelWidth/{print $2}')
sips --cropToHeightWidth $W $W week-$WK-sweet.jpg > /dev/null
sips -z 1080 1080 week-$WK-savory.jpg > /dev/null
sips -z 1080 1080 week-$WK-sweet.jpg > /dev/null
```

**Caption parsing:**

Caption convention from `AUTOMATION.md`:

```
Specials!!

<Savory Name>
<Savory description, possibly multi-line>

<Sweet Name>
<Sweet description, possibly multi-line>
```

Sometimes Instagram caption has price embedded inline (e.g. `— $17.95`). User has stated: **if price not in caption, do NOT render a price** — Sections.jsx guards on `s.price` truthy. Set `price` only when present in the post; otherwise omit the field entirely (not empty string, not `null`).

**Veg flag:** set `veg: true` for any item without meat/fish. Inspect caption — French toast / pancakes / vegetarian dishes → true. Anything with sausage/bacon/eggs-with-meat → false. When ambiguous, ask the user.

**Update `data.js`:**

```js
sourcePost: "https://www.instagram.com/p/<shortcode>/",
...
- Open `http://localhost:5178/#specials` (use the `flytrap` launch config).
- DOM probe each `<image-slot>`: `shadowRoot.querySelector('img').src` must end with the WEEK's filename, `naturalWidth >= 800`, `complete === true`.
- Visually confirm each card's image MATCHES the dish described in the caption (e.g. curry eggs photo lines up with "Darjeeling Limited"). If wrong, you pulled the wrong slide.

**Commit message:**

```
feat(specials): pull <week> Instagram specials
